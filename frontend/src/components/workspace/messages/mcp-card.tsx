...
function getLucideIcon(iconName: string): LucideIcon | undefined {
  const pascalName = iconName
    ...
    .join("");
  return (lucideIcons as unknown as Record<string, LucideIcon | undefined>)[
    pascalName
  ];
}
...
function parseMCDPointsProducts(
  raw: Record<string, unknown>,
  userPoints?: number,
): Record<string, unknown> {
...
function parseMCDMeals(
  ...
  const parsedCategories = categories.map((cat) => {
    const meals = (cat.meals as Array<Record<string, unknown>> | [])?.map(
      (meal) => {
...
function parseMCDResult(
  toolName: string,
  raw: Record<string, unknown>,
  args: Record<string, unknown>,
): Record<string, unknown> {
  if (
    toolName.includes("mall-points-products") ||
    toolName.includes("points")
  ) {
...
export function MCPCard({ toolName, args, result, onAction }: MCPCardProps) {
  ...
  if (toolName.includes("weather")) {
    ...
    toolName.includes("mcd") ||
    toolName.includes("mall") ||
    toolName.includes("account") ||
    toolName.includes("coupon") ||
