...
class TestThreadMetadataInjection:
    ...
    def test_create_thread_injects_user_id_metadata(self):
        ...
        async def go():
            ...
            inbound = InboundMessage(channel_name="test", chat_id="chat1", user_id="user-123", text="hi")
    ...
    def test_new_command_injects_user_id_metadata(self):
        ...
        async def go():
            ...
            inbound = InboundMessage(channel_name="test", chat_id="chat1", user_id="user-456", text="/new")
            await bus.publish_inbound(inbound)
            await _wait_for(lambda: len(outbound_received) >= 1)
            await manager.stop()
