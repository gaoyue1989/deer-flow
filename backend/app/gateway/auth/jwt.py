...
def get_algorithm() -> str:
    """Get JWT algorithm from config or environment variable (cached).

    Priority:
    1. config.yaml multi_tenant.algorithm
    2. DEER_FLOW_JWT_ALGORITHM environment variable
    3. Default algorithm (HS256)
    ...
    if algorithm:
        _algorithm_cache = algorithm
        return algorithm

    algorithm = os.getenv("DEER_FLOW_JWT_ALGORITHM")
    if algorithm:
        _algorithm_cache = algorithm
        return algorithm

    _algorithm_cache = DEFAULT_ALGORITHM
    return DEFAULT_ALGORITHM


def reset_algorithm_cache() -> None:
    """Reset the cached JWT algorithm."""
    global _algorithm_cache
