...
class FileMemoryStorage(MemoryStorage):
    ...
    def _get_memory_file_path(self, agent_name: str | None = None, user_id: str | None = None) -> Path:
        ...
        Priority:
        1. Per-user memory: {base_dir}/memory/user_{user_id}/memory.json
        2. Per-agent memory: {base_dir}/memory/agent_{agent_name}/memory.json
        3. Global memory: {base_dir}/memory.json (or custom storage_path)
        """
        if user_id is not None:
            return get_paths().base_dir / "memory" / f"user_{user_id}" / "memory.json"
